...
py = (100-25*sin(5*t))*sin(t)*sqrt(1-(t/(5*pi))^2);
pz = t;

% 计算得到的反解结果
theta1 = atan2(py, px);
s1 = sin(theta1); c1 = cos(theta1);fprintf("theta1 done\n");

k = ((sqrt(px^2+py^2)-a1)^2+pz^2-d4^2-a2^2-a3^2)/(2*a2);
theta3p = simplify(atan2(a3, d4)-atan2(k, real(sqrt(a3^2+d4^2-k^2))));
theta3n = simplify(atan2(a3, d4)-atan2(k, -real(sqrt(a3^2+d4^2-k^2))));
s3p = sin(theta3p); c3p = cos(theta3p); fprintf("theta3p done\n");
s3n = sin(theta3n); c3n = cos(theta3n); fprintf("theta3n done\n");

theta2p = simplify(atan2((a2*py*s3p-d4*py)*s1+a1*d4-a3*pz-a2*c3p*pz-a1*a2*s3p-c1*d4*px+a2*c1*px*s3p, ...
    (a3*py+a2*c3p*py)*s1+a3*c1*px-d4*pz-a1*a2*c3p-a1*a3+a2*pz*s3p+a2*c1*c3p*px)-theta3p);
theta2n = simplify(atan2((a2*py*s3n-d4*py)*s1+a1*d4-a3*pz-a2*c3n*pz-a1*a2*s3n-c1*d4*px+a2*c1*px*s3n, ...
...
theta4p = simplify(atan2(-ax*s1+ay*c1, -ax*c1*c23p-ay*s1*c23p+az*s23p));
theta4n = simplify(simplify(atan2(-ax*s1+ay*c1, -ax*c1*c23n-ay*s1*c23n+az*s23n)));
s4p = sin(theta4p); c4p = cos(theta4p); fprintf("theta4p done\n");
s4n = sin(theta4n); c4n = cos(theta4n); fprintf("theta4n done\n");

theta5p = simplify(atan2(-(ax*(s1*s4p+c1*c4p*c23p)+ay*(s1*c4p*c23p-c1*s4p)-az*(c4p*s23p)), ...
    -ax*c1*s23p-ay*s1*s23p-az*c23p));
theta5n = simplify(atan2(-(ax*(s1*s4n+c1*c4n*c23n)+ay*(s1*c4n*c23n-c1*s4n)-az*(c4n*s23n)), ...
...
theta6p = simplify(atan2(-nx*(c1*s4p*c23p-s1*c4p)-ny*(c1*c4p+s1*s4p*c23p)+nz*s4p*s23p, ...
    nx*(s1*s4p*c5p-c1*s5p*s23p+c1*c4p*c5p*c23p)+ny*(s1*c4p*c5p*c23p-s1*s5p*s23p)-nz*(s5p*c23p+c4p*c5p*s23p)));
theta6n = simplify(atan2(-nx*(c1*s4n*c23n-s1*c4n)-ny*(c1*c4n+s1*s4n*c23n)+nz*s4n*s23n, ...
...
% 轨迹三维图
figure(1);
fplot3(px, py, pz, [0, 5*pi]); grid on;
title("末端执行器轨迹");

% 关节变量图
figure(2);
subplot(2,3,1);
fplot(t, rad2deg(theta1), [0, 5*pi]); grid on;
xlabel('t'), ylabel('\theta_{1}');
title('\theta_{1}');

subplot(2,3,2);
fplot(t, rad2deg(theta2p), [0, 5*pi], '-b'); grid on;
hold on;
fplot(t, rad2deg(theta2n), [0, 5*pi], '-r'); grid on;
xlabel('t'), ylabel('\theta_{2}');
title('\theta_{2}');

subplot(2,3,3);
fplot(t, rad2deg(theta3p), [0, 5*pi], '-b'); grid on;
hold on;
fplot(t, rad2deg(theta3n), [0, 5*pi], '-r'); grid on;
xlabel('t'), ylabel('\theta_{3}');
title('\theta_{3}');

subplot(2,3,4);
fplot(t, rad2deg(theta4p), [0, 5*pi], '-b'); grid on;
hold on;
fplot(t, rad2deg(theta4n), [0, 5*pi], '-r'); grid on;
xlabel('t'), ylabel('\theta_{4}');
title('\theta_{4}');

subplot(2,3,5);
fplot(t, rad2deg(theta5p), [0, 5*pi], '-b'); grid on;
hold on;
fplot(t, rad2deg(theta5n), [0, 5*pi], '-r'); grid on;
xlabel('t'), ylabel('\theta_{5}');
title('\theta_{5}');

subplot(2,3,6);
fplot(t, rad2deg(theta6p), [0, 5*pi], '-b'); grid on;
hold on;
fplot(t, rad2deg(theta6n), [0, 5*pi], '-r'); grid on;
